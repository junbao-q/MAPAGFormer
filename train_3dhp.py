...
from loss.pose3d import (loss_mpjpe, n_mpjpe, loss_velocity,
                         loss_limb_var, loss_limb_gt, loss_angle,
                         loss_angle_velocity)
from utils.data import denormalize
from data.reader.motion_dataset import MPI3DHP, Fusion
from utils.tools import (set_random_seed, get_config, print_args,
...
def parse_args():
    ...
    parser.add_argument("--config", type=str,
                        default="configs/mpi/MotionAGFormer-large.yaml",
                        help="Path to the config file.")
    parser.add_argument('-c', '--checkpoint', type=str, metavar='PATH',
                        help='checkpoint directory')
    parser.add_argument('--checkpoint-file', type=str,
                        help="checkpoint file name")
    parser.add_argument('--new-checkpoint', type=str, metavar='PATH',
                        default='mpi-checkpoint',
...
def evaluate(model, test_loader, n_frames):
    ...
    for data in tqdm(test_loader, desc='eval'):
        ...
        [input_2D, gt_3D, batch_cam, scale, bb_box] = \
            get_variable('test', [input_2D, gt_3D, batch_cam, scale, bb_box])
        N = input_2D.size(0)
        ...
        output_3D = output_3D * scale.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) \
                               .repeat(1, output_3D.size(1), 17, 3)
        pad = (n_frames - 1) // 2
        ...
        for seq_cnt in range(len(seq)):
            ...
            if seq_name in data_inference:
                ...
                data_inference[seq_name] = inference_out[seq_cnt] \
                    .permute(2, 1, 0).cpu().numpy()
...
def save_data_inference(path, data_inference, latest):
    mat_path = os.path.join(path,
                            'inference_data.mat' if latest
                            else 'inference_data_best.mat')
...
def train(args, opts):
    ...
    epoch_start = 0
    min_mpjpe = float('inf')
    wandb_id = opts.wandb_run_id if opts.wandb_run_id else wandb.util.generate_id()
    ...
    if opts.checkpoint:
        ...
        if os.path.exists(checkpoint_path):
            ...
            print("[WARN] Checkpoint path not found. Start from scratch.")
            opts.resume = False
