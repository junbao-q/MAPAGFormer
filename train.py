...
from loss.pose3d import loss_mpjpe, n_mpjpe, loss_velocity, loss_limb_var, loss_limb_gt, loss_angle, \
    loss_angle_velocity,loss_anatomy_soft_constraint 
...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/h36m/MAPAGFormer-small.yaml", help="Path to the config file.")
...
def train_one_epoch(args, model, train_loader, optimizer, device, losses, iters_to_accumulate=None):
    if iters_to_accumulate is None:
        iters_to_accumulate = getattr(args, 'accumulation_steps', 1)
    model.train()
    optimizer.zero_grad() 
    ...
    for i, (x, y) in enumerate(tqdm(train_loader)):
        ...
        loss_anatomy = loss_anatomy_soft_constraint(pred)
        ...
        loss_total = (loss_3d_pos + \
                     ...
                     args.lambda_anatomy * loss_anatomy      
        ...
        losses['anatomy'].update(loss_anatomy.item(), batch_size)  
        losses['total'].update(loss_total.item() * iters_to_accumulate, batch_size)
...
def train(args, opts):
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        loss_names = ['3d_pose', '3d_scale', '2d_proj', 'lg', 'lv', '3d_velocity', 'angle', 'angle_velocity','anatomy','total']
        ...
        if opts.use_wandb:
            wandb.log({
                ...
                'train/loss_anatomy': losses['anatomy'].avg,
                'eval/mpjpe': mpjpe,
