...
def loss_anatomy_soft_constraint(x, weight=1.0):
    ...
    pred_angles = get_angles(x) 
    batch_size, time_steps, num_angles = pred_angles.shape
    
  
    total_angle_loss = torch.tensor(0.0, device=x.device)
    
   
    for angle_idx, (low_bound, high_bound) in valid_angle_ranges.items():
      
        current_angles = pred_angles[..., angle_idx]  
      
        below_low = torch.clamp(low_bound - current_angles, min=0.0)  
        loss_below = torch.square(below_low)
        
        
        above_high = torch.clamp(current_angles - high_bound, min=0.0)  
        loss_above = torch.square(above_high)
        
        
        total_angle_loss += weight * (torch.mean(loss_below) + torch.mean(loss_above))
    

    num_constrained_angles = len(valid_angle_ranges)
    if num_constrained_angles > 0:
        total_angle_loss /= num_constrained_angles
